...
function OwnerCreatePage() {
  ...
  const formik = useFormik<OwnerInterface>({
    initialValues: {
      created_date: new Date(new Date().toDateString()),
      status: '',
      role: '',
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="created_date" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Created Date
            </FormLabel>
            <DatePicker
              selected={formik.values?.created_date ? new Date(formik.values?.created_date) : null}
              onChange={(value: Date) => formik.setFieldValue('created_date', value)}
            />
          </FormControl>
